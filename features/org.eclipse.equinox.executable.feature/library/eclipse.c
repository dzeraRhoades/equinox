...
static const _TCHAR * getVMArg( _TCHAR **vmArgs, _TCHAR* argName );
static void     getVMCommand( int launchMode, int argc, _TCHAR* argv[], _TCHAR **vmArgv[], _TCHAR **progArgv[] );
static int 		determineVM(_TCHAR** msg, int minJavaVersion);
...
static int _run(int argc, _TCHAR* argv[], _TCHAR* vmArgs[])
{
	...
	handleVMArgs(&vmArgs);
	int javaMinVersion = 0;
	// getting java min version from ini file or command line args
	_TCHAR* versionStr = getVMArg(vmArgs, _T_ECLIPSE("requiredJavaVersion"));
	if(versionStr != NULL)
#ifndef _UNICODE
		javaMinVersion = atoi(versionStr);
#else
		javaMinVersion = _wtoi(versionStr);
#endif

    launchMode = determineVM(&msg, javaMinVersion);
    if (launchMode == -1) {
    	/* problem */
    ...
    while(running)
    {
		...
		if(launchMode == LAUNCH_JNI) {
			_tprintf(_T_ECLIPSE("java was started with: %s\n"), jniLib);
...
/* retruns the value of argName parameter in vmArgs */
static const _TCHAR*  getVMArg( _TCHAR** vmArgs, _TCHAR* argName ) {
	_TCHAR *_arg = NULL;
	_TCHAR **_args = vmArgs;
	while((_arg = *_args) != NULL)
	{
		// if we find argument
		if(_tcsstr(_arg, argName) != NULL)
		{
			_TCHAR* value = _tcsstr(_arg, _T_ECLIPSE("=")) + 1;
			return value;
		}
		_args++;
	}
...
static int determineVM(_TCHAR** msg, int minJavaVersion) {
    ...
    if (vmName == NULL) {
		...
		/* trying to find java in inventory */
		_TCHAR* invJavaHome = NULL;
		findJavaInInventory(minJavaVersion, &invJavaHome);
		// if java was found in inventory
		if(invJavaHome != NULL) {
			_tprintf(_T_ECLIPSE("inv java found: %s\n"), invJavaHome);
			_TCHAR* binPath = _T_ECLIPSE("/bin/\0");
			// if the path ends with dir separator
			if(lastDirSeparator(invJavaHome) + 1 == 0) {
				binPath++;
			}
			
			javaVM = malloc((_tcslen(invJavaHome) + _tcslen(binPath) + _tcslen(defaultJava) + 10) * sizeof(_TCHAR));
			_stprintf( javaVM, _T_ECLIPSE("%s%s%s"), invJavaHome, binPath, defaultJava );
			free(invJavaHome);
		}
		else {
