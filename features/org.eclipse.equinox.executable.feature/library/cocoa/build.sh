...
while [ "$1" != "" ]; do
    if [ "$1" = "-os" ] && [ "$2" != "" ]; then
        ...
        programOutput="$2"
        shift
    elif [ "$1" = "-rt_bin" ] && [ "$2" != "" ]; then
        rt_equinox_binaries_loc="$2"
        shift
    else
...
EXEC_DIR="$rt_equinox_binaries_loc/org.eclipse.equinox.executable"
...
LIBRARY_OUTPUT_DIR="$rt_equinox_binaries_loc/org.eclipse.equinox.launcher.$defaultWS.$defaultOS.$defaultOSArch"
...
export PROGRAM_OUTPUT DEFAULT_OS DEFAULT_OS_ARCH DEFAULT_WS ARCHS PROGRAM_OUTPUT_DIR LIBRARY_OUTPUT_DIR JAVA_HEADERS 
